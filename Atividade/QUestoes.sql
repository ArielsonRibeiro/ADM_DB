...
CREATE TRIGGER TG_ALTER_GRADE_CURSO_CALCULAR_CREDITOS ON [Curriculo]
...
BEGIN
    ...
    UPDATE SIGAA.dbo.[Curso]
        SET TOT_CRED = (Select SUM(disc.QTD_CRED)
            FROM Curriculo AS cur 
...
CREATE TRIGGER TG_SETUPPER_DSC_CURSO ON [Curso]
    FOR INSERT, UPDATE 
AS
    BEGIN
        UPDATE Curso
...
CREATE TRIGGER TG_VALIDAR_COORDENADOR_CURSO ON [Curso]
    FOR INSERT, UPDATE 
AS
    IF ( UPDATE(COD_COORD) AND (SELECT COUNT(COD_COORD) FROM inserted) != 0)
        BEGIN
            ...
            SELECT @COD_CURSO_PROF = COD_CURSO FROM SIGAA.dbo.Professor;
...
-- 5. A mudança na quantidade de créditos das Disciplina só pode ser realizada se a mesma ainda
...
CREATE TRIGGER TG_SET_UPPER_NOME_DCIS ON [Disciplina]
    FOR INSERT, UPDATE 
AS
    IF (UPDATE(NOM_DISC))
        BEGIN
...
-- Turma matriculadas de cada aluno;
    ...
    BEGIN
        ...
        IF EXISTS (SELECT * FROM INSERTED)
            BEGIN
                ...
                SELECT @VAG_OCUP = VAG_OCUP FROM SIGAA.dbo.Turma WHERE COD_DISC = @COD_DISC AND ANO = @ANO AND SEMESTRE = @SEMESTRE AND TURMA = @TURMA;
...
-- 12. Um professor pode lecionar o máximo de 5 Turma por semestre;
CREATE TRIGGER TG_VALIDAR_TOTAL_TURMA_PROFESSOR ON Turma
FOR INSERT, UPDATE
AS
    BEGIN
        DECLARE 
              @COD_PROF INT
            , @SEMESTRE SMALLINT;
            SELECT @COD_PROF = @COD_PROF FROM Turma;

        IF ( (SELECT COUNT(*) FROM Turma
...
-- nenhuma inconsistência em relação aos currículos cadastrados, os históricos dos Aluno e as
-- Disciplina matriculadas.

CREATE TRIGGER TG_VALIDAR_PRE_REQUISITO ON [PreRequisito]
FOR INSERT, UPDATE
AS
    BEGIN
        DECLARE 
        ...
        IF EXISTS (SELECT * FROM HistoricoEscolar HE1
                            WHERE COD_DISC = @COD_DISC AND NOT EXISTS (SELECT * FROM HistoricoEscolar HE2 
                                                                        WHERE HE2.MAT_ALU = HE1.MAT_ALU AND HE2.COD_DISC = @COD_DISC_PRE))
            BEGIN
                RAISERROR('CONFLITO ENTRE O PRÉ REQUISITO E OS HISTORICOS DOS Aluno', 10 , 1 )
                ROLLBACK;
            END
        IF EXISTS (SELECT * FROM Curriculo C1
                            WHERE COD_DISC = @COD_DISC AND NOT EXISTS (SELECT * FROM Curriculo C2
                                                                        WHERE C2.COD_CURSO = C1.COD_CURSO AND C2.COD_DISC = @COD_DISC_PRE))
            BEGIN
                RAISERROR('CONFLITO ENTRE O PRÉ REQUISITO E AS Curriculo DOS Curso', 10 , 1 )
                ROLLBACK;
            END

        IF EXISTS (SELECT * FROM TurmaMatriculada T1
                            WHERE T1.COD_DISC = @COD_DISC AND NOT EXISTS (SELECT * FROM TurmaMatriculada T2
                                                                            WHERE T2.MAT_ALU = T1.MAT_ALU  
                                                                            AND T2.ANO = T1.ANO
                                                                            AND T2.SEMESTRE = T1.SEMESTRE
                                                                            AND T2.COD_DISC = @COD_DISC_PRE
                                                                            OR NOT EXISTS (SELECT * FROM HistoricoEscolar HE2 
                                                                                WHERE HE2.MAT_ALU = T1.MAT_ALU AND HE2.COD_DISC = @COD_DISC_PRE) ) )
            BEGIN
                RAISERROR('CONFLITO ENTRE O PRÉ REQUISITO E AS Turma MATRICULADAS', 10 , 1 )
                ROLLBACK;
            END
...
CREATE TRIGGER TG_UPDATE_ALUNO ON [Aluno]
FOR UPDATE
AS
    IF(UPDATE(COD_CURSO))
    BEGIN
        ...
        IF ( (SELECT COUNT(*) FROM SIGAA.dbo.[HistoricoEscolar] WHERE MAT_ALU = @MAT_ALU) > 0
            OR (SELECT COUNT(*) FROM SIGAA.dbo.[TurmaMatriculada] WHERE MAT_ALU = @MAT_ALU) > 0 )
            BEGIN
...
-- 16. A idade mínima para Aluno é de 16 anos. CHECK

-- 17. Um aluno não pode realizar matrícula em uma disciplina para a qual não possua o pré-
-- requisito;
CREATE TRIGGER TG_VALIDAR_REQUISITO_DISCIPLINA ON [TurmaMatriculada]
FOR INSERT
AS
    BEGIN
    DECLARE
...
-- 18. A matrícula em Disciplina já cursadas (aprovadas) não pode ser feita;
CREATE TRIGGER TG_VALIDAR_DISCIPLINA_JA_CURSADA ON [TurmaMatriculada]
FOR INSERT
AS
    BEGIN
    DECLARE
    ...
    print SELECT * FROM [HistoricoEscolar] WHERE COD_DISC = @COD_DISC AND MAT_ALU = MAT_ALU AND SITUACAO = 'AP');
    IF(EXISTS (SELECT * FROM [HistoricoEscolar] WHERE COD_DISC = @COD_DISC AND MAT_ALU = MAT_ALU AND SITUACAO = 'AP'))
        BEGIN
...
CREATE TRIGGER TG_VALIDAR_ORDEM_NOTAS ON [TurmaMatriculada]
FOR INSERT, UPDATE
AS
    IF(UPDATE(NOTA_3))
        BEGIN
...
-- 27. Não podem ser lançadas Disciplina que não pertençam a grade curricular do aluno;
CREATE TRIGGER TG_VALIDAR_DISCIPLINA_PERTENCE_AO_CURSO_DO_ALUNO_LANCAR_HISTORICO ON [HistoricoEscolar]
FOR INSERT
AS
    BEGIN
    DECLARE
...
CREATE TRIGGER TG_FINALIZAR_TURMA ON [TurmaMatriculada]
FOR INSERT, UPDATE
AS
    BEGIN
    DECLARE
